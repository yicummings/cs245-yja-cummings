...
public class BST<T extends Comparable> {
    ...
    public List<T> rangeQuery(T lowerBound, T upperBound){
        ...
        rangeQueryEval(root, lowerBound,upperBound,values);
        return values;
    }
    private void rangeQueryEval(BSTNode<T> node,T lowerBound, T upperBound, List<T>result) {
    	if(node==null) {
    		return;
    	}
    	...
    	if (node.item.compareTo(lowerBound)>0) {
    		rangeQueryEval(node.left,lowerBound,upperBound,result);
    	}
    	if (node.item.compareTo(upperBound)<0) {
    		rangeQueryEval(node.right,lowerBound,upperBound,result);
    	}
    }
